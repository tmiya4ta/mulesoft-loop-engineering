...
## `group-id` を他プロジェクトから写さない。**別の組織に publish する事故になる**

**症状**: `/mule-init` の直後、エージェントが「`group-id` など**他プロジェクトでの実例を確認します**」
と言って、同じマシンにある別プロジェクトの pom を見に行こうとした。

**なぜ危ないか**: CH2 / RTF は Exchange 経由でしか置けず、**Exchange のアセットは
groupId = Anypoint の組織 ID** という決まりです。写した組織 ID が違えば、publish が落ちるか、
**別の組織に publish されます**。落ちるなら気付きますが、通ってしまう方が厄介です。

**直し方**: 人に聞く。または `anypoint-cli-v4 account business-group list`。
**分からなければ空のまま止まって聞く。推測で埋めない。**

**同じ形の広い問題**: このプラグインの gotchas は「根拠: <プロジェクト名> で実測」と書いてあり、
**それは引用であって「そこを見に行け」ではありません。** 引用元は利用者のマシンには存在しません。
エージェントが根拠のプロジェクト名を「参照先」と読むと、無いものを探すか、
**あった場合はもっと悪く、他人の設定を写します。**

**読んでよい範囲を決めるのが対策です**: (1) このリポジトリの中 と (2) プラグイン
(`plugin-root.sh` で解決したパス) だけ。`git rev-parse --show-toplevel` より上に出ない。

根拠: **2 回**。(1) v0.6.34 の素通し試験の初回、`/mule-init` の直後に別プロジェクトの pom を
見に行こうとした。README に別プロジェクト名を書いた不注意が引き金だったが、`mule-init` に
組織 ID の出どころが書いていなかったのも穴だった。(2) それを直した直後、**同じことが再発**した。
今度の引き金は**置き場所**で、試験用ディレクトリを既存プロジェクトの隣、しかも**同じ git
リポジトリの中**に作っていた (`git rev-parse --show-toplevel` が親を返し、隣に 27 個の
プロジェクトが見える状態)。**隣にあるものは読まれます。** リポジトリを分けたうえで、
`mule-init` と `CLAUDE.md` に読んでよい範囲を書いた (2026-09-10)。
...
**cache がいつ作られるかは外から見えません。** 「セッション開始時にだけ」と書いていましたが、
会話の途中に `0.6.25` (22:09) と `0.6.29` (22:26) のディレクトリが増えるのを観測したので取り消しました。
**ディレクトリが増えても、有効な hook 設定は変わりません** — 上の 2 つがその証拠です。
分かっているのは「push した瞬間には無い」と「増えても効かない」の 2 つだけです。
