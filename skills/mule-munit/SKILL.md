...
- **コネクタが返す値の型。** `then-return` に書くのは自分で用意した値なので、実物の型とは無関係です。
  実例: Oracle の TIMESTAMP 列は素の `Object` で来るため `as String` が落ちるのに、モックが文字列を
  返していたので **15 スイート全部緑**でした (落ちたのは配備して curl を当てたとき)。
  **モックを実物に似せて直そうとしない** — 実物の型が分からないから間違えるので。
  型に依存する変換を dwl から追い出す (SQL 側で文字列にする) 方で直します。詳細は `gotchas/db.md`。
