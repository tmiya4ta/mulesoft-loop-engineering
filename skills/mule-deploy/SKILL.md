...
4. 疎通は `python3 scripts/smoke-check.py --no-basepath <ゲートウェイの URL>`。
   ...
   **配置先が共有の実 DB なら `--idempotent-only` を付ける。** 実データを書き換えるケース
   (PUT / POST / DELETE) は 1 回目と 2 回目で結果が変わり (数量の実値化、楽観ロックの 409、
   検索の件数)、**exit code で判定できなくなって人が毎回差分を読む検査に戻ります**
   (実測: inventory3-api T-006 は exit 1 のまま「前回と同じ分類だから」で passed にしていた)。
   書き込みの正しさは MUnit (mock なので決定的) が持ち、配置先で確かめたいのは接続・経路・変換
   なので、GET だけでも目的は果たせます。**冪等なケースが 1 件も無ければ exit 2** です
   (0 件の成功は成功ではない)。

   **method だけでは冪等性は決まりません。** GET でも、前に流した書き込みの累積を拾えば
   結果が変わります (検索の件数など)。落ちたケースは 2 つに分けて、**違う扱いをしてください**:
   ...
   | **実データが動いたから落ちる** (検索の件数が baseline とズレた) | **外さない。samples を直す。** 期待値が baseline に依存しているのが原因なので、実データの変化に影響されない形 (特定 ID を取る、件数を数えない) にする |

   **2 つ目を `.smoke-skip` で隠さないでください。** 隠すと、同じ場所で起きる本物の退行も
   一緒に見えなくなります。`.smoke-skip` は理由が空だと効かず、外した数が半分を超えると警告します。
