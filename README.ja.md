...
v0.6.52 の 2 分類は実機で両方さばけました (`--idempotent-only` + `.smoke-skip` で `exit 0`)。
その過程で、**もっと手前で防げたはずのこと**が分かったので規則にしました。

**サンプルは MUnit (mock) だけでなく、配置先の実 DB にも当たります。** なのに「条件に合う行を全部
返す」形の入力で作ると、**実データが増減するたびに期待値とズレて落ちます** — しかも落ちた理由が
退行なのかデータの変化なのか区別できません (実測: 検索のサンプルが `totalCount 1 → 2` で落ち、
原因を追うのに往復しました)。`/mule-start` の受け入れ条件を作る段に、**入力は結果が 1 つに決まる形に
する**(特定の ID やコードで絞る、件数を期待値に入れない、そのケース専用の行を使う) を入れました。

もう 1 つ。**`in.json` を変えるのは人の承認が要ります。** 「期待値は変えない」は `out.json` の話で、
入力条件はそれに当たりません。しかし**受け入れ条件そのものが変わる**ので、エージェントの判断で
変えてよいものではありません。実機側のセッションは今回それを自分で判断して承認を取ってから
変えました。**その判断の方が規則より正しかった**ので、規則の側を合わせました
(`docs/methodology.md` の原則 2 と `template/CLAUDE.md`)。

</details>

<details>
<summary><b>v0.6.52</b> — **冪等性は method だけでは決まらない。** 落ちる 2 分類を分けて扱う (`.smoke-skip`)</summary>

v0.6.51 の `--idempotent-only` を実機で回してもらったら、**まだ 2 件落ちました。** 報告が正確だった
ので設計の穴がはっきりしました: **GET でも、前に流した書き込みの累積を拾えば結果が変わります。**

落ちる理由は 2 つあり、**同じ扱いにしてはいけません**:

| 落ちる理由 | 扱い |
|---|---|
| **配置先では原理的に測れない** (fault-injection の 502 は、実の upstream が健全な限り出せない) | `<case>.smoke-skip` に理由を 1 行書いて外す。**その枝は MUnit が mock で持つもの**で、smoke の役目ではない |
| **実データが動いたから落ちる** (検索の件数が baseline とズレた) | **外さない。samples を直す。** 期待値が baseline に依存しているのが原因なので、実データの変化に影響されない形にする |

2 つ目を外して緑にすると、**同じ場所で起きる本物の退行も一緒に隠れます。** そこで `.smoke-skip` には
歯止めを付けました:

- **理由が空のファイルは効きません** (「とりあえず置いて黙らせる」ができない)
- 外したケースは名前と理由が毎回出力に並ぶ
- **外した数が全体の半分を超えたら警告**します (緑のまま中身が空になるのが一番危ない)

4 状態で実測しました: 宣言なし → 落ちる / 理由が空 → 外れない / 理由あり → 外れて exit 0 /
半分超え → 警告。
...
v0.6.45 の `ingress: gateway` を利用者の Sandbox で実際にやってみました (`ch2-public-url.py --remove` の実測)。
その過程で、**ゲートウェイ経由は一度も通っていなかった**ことが分かりました。

| 見つかったもの | 中身 |
|---|---|
| **upstream は末尾スラッシュが要る** | ゲートウェイは待ち受けパス (`/inventory3-api/`) を剥がした残り (`inventory`、先頭スラッシュ無し) を upstream の URI に**そのまま連結**します。`https://app/api` だと `https://app/apiinventory` になり `404 No listener for endpoint: /apiinventory`。`/api/` にしたら 200 で在庫が返りました |
| **適用・解除は数秒〜十数秒遅れて効く** | 201 / 204 の直後に叩くと**前の状態**が返ります (外した直後の 1 回目は 401 のまま、2 回目から 200。付け直した直後の 1 回目は 200、2 回目から 401) |

**なぜ気付けなかったか。** client-id-enforcement は契約が無いと upstream に届く前に 401 を返します。
つまり **「認証なしで 401」を見て「経路ができた」と判断していた**が、その裏で upstream は壊れていました。
確かめるには、ポリシーを一時的に外して素通しで 200 を見て、すぐ同じ設定で付け直すしかありません。

**`policy-check.sh` は結果が落ち着くまで待つようにしました** (同じ結果が 2 回続くまで、最大 5 回・8 秒間隔)。
待たずに 1 回で判定すると「効いていない」「まだ守れている」を取り違えます。

`/mule-deploy` には、**既に `public` で動いているものを `gateway` に切り替えるときの順番**を書きました:
先に upstream を内部 URL (末尾スラッシュ付き) へ向け替え、**それから**公開 URL を外す。逆にすると経路ごと切れます。

実機での結果 (利用者の Sandbox、2026-09-12): upstream を内部 URL に向け替え → ポリシーを一時的に外して
素通しで 200 を確認 → 同じ設定で付け直して 401 → `--remove` で公開 URL を削除 → **直接は 404、
ゲートウェイ経由は 401、アプリは RUNNING**。迂回路が塞がりました。
