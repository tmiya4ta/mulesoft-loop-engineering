...
既存プロジェクトにスクリプトを配り直したとき、**`inventory2-api` が git リポジトリではない**ことに
気付きました。`knowledge/` も `tasks/` も揃っていて、6 ゴールを回した実績があります。それでも:

- 実行エージェントの **worktree 隔離** — 作れないので、並列ゴールが同じ木を踏み合う
- **`wave-guard.sh` の追記型ファイルの分担** — git が無ければ素通りする設計 (worktree を誤爆しないため)
- **「K ファイルが diff に含まれているか」の確認** — diff が取れないので常に真とも偽とも言えない
- **`/mule-learn --share` の PR** — 昇格が共有されない

の 4 つが、**エラーも警告も出さずに何もしていませんでした**。このプラグインが今までやってきたことの
逆です。`deploy-guard` は deny を返し、`wave-guard` は deny を返し、`coverage-check` は exit 1 を返す。
**黙って効かない検査は、無い検査より悪い** — 効いていると思って進むからです。

`scripts/preflight.sh` の最初に git の確認を入れ、無ければ **exit 2 で 1 件も配らない**ようにしました
(pom.xml の確認と同じ扱い)。落ちるときは上の 4 つを名前で並べて `git init` の 1 行を出します。
「なぜ止まったか」を人が推測しなくていいように。
