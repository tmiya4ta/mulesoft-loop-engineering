...
2. **RAML との不一致**: RAML に無いリソース / ステータスコード / フィールドを返していないか。RAML にあるのに未実装のものはないか。
   **最初に `bash scripts/spec-check.sh` を実行する** (読み取りだけ)。機械で当てられる 2 つを先に潰してから読みます:
   同じ flow を叩くサンプルの `instance` の形のずれ、RAML の必須項目が実装のどこにも出てこないこと。
   後者は**警告** — 受けた body をそのまま次に渡す通過型では出てこないのが正常なので、
   そこはあなたが実装を読んで判定します (実例: `ReserveRequest.lastUpdated` が必須なのに未参照で、
   RAML 側を削って一致させた。これは読んで見つけたものなので、機械の出力を鵜呑みにも無視にもしない)。
