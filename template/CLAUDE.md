...
- デプロイは `/mule-deploy` だけが行い、`context/deployment/authorizations.yaml` の `deploy.sandbox` が `allowed` で、台帳に `stage: deploy` のゴールがあるときに限る。**会話での言い直しは求めない** (許可はファイルに書いてあり、二度確かめない)。可否は `deploy-guard.py` が hook で判定するので、deny が返ったら理由をそのまま人に伝えて止まる。回避経路を探さない。本番は人が行う。
