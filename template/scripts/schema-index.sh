...
# 手で書き写すと版がずれ、gotchas/build.md の「コネクタの GAV を推測しない」を人間の側で破ることになる。
#
# **生成物は .gitignore に入れないこと。** `isolation: "worktree"` の worktree は origin/main から
# 切られるので、追跡されていないファイルは実行エージェントに一切届きません。届かないと
# 「要素名を推測しない」という指示だけが残り、推測する以外に手が無くなります
# (このスクリプトを作った理由そのものが消える)。/mule-run は配る前に `git add -A` するので、
# 除外しなければ自動的に入ります。
