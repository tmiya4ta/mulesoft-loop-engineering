...
#
# **候補の順ではなく plugin.json の版で選ぶ。** cache は git push と同時には作られないので、
# push 済みでも走っているセッションの cache には現れないことがある。
# (**作られる契機は外から見えません。** 「セッション開始時にだけ」と書いていたが、
#  会話の途中に 0.6.25 / 0.6.29 のディレクトリが増えるのを観測したので取り消した。
#  分かっているのは「push した瞬間には無い」ことだけ。) 実測 (2026-09-10、v0.6.15):
