...
#         run-log.sh deploy cloudhub2 Sandbox ok|failed 210
set -u
mkdir -p knowledge
...
case "${1:-}" in
  ...
  deploy)   printf '{"ts":"%s","event":"deploy","kind":"%s","environment":"%s","result":"%s","seconds":%s}\n' "$ts" "${2:-}" "${3:-}" "${4:-}" "${5:-0}" ;;
  *) echo "usage: run-log.sh dispatch|done|review|deploy ..." >&2; exit 2 ;;
